...
func GetKeyNonBlocking() (rune, Key, error) {
	if !isOpen {
		panic("function GetKeyNonBlocking() should be called after Open()")
	}
	waitingForKey = true
	defer func() { waitingForKey = false }()
...
func GetSingleKeyNonBlocking() (ch rune, key Key, err error) {
	err = Open()
	if err == nil {
		ch, key, err = GetKeyNonBlocking()
	}
	return
}
